...
// Formats basic markdown elements (bold, links, code, bullet lists, newlines) safely
  ...
  return (
    <div className="message-formatted-content">
      {lines.map((line, lineIdx) => {
        ...
        const parts = [];
        // Regex to capture bold (**bold**), code (`code`), or markdown links ([text](url))
        ...
        if (isBullet) {
          return (
            <div key={lineIdx} className="chat-bullet-item">
              ...
              <span className="chat-bullet-text">{parts}</span>
            </div>
          );
        }

        return <p key={lineIdx} className="chat-paragraph">{parts}</p>;
      })}
    </div>
  );
};
